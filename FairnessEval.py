...
def DataDescription(X_test,y_test,class_names,column_names,protected_values,protected_feature):
    ...
    # Group the data based on the unique values of the column
...
def standardMetricsPerGroup(clfs,column_names,target,X_test,y_test,protected_feature,protected_values):
    ...
    #for i in range(len(clfs)):
        ...
        for i in range(len(by_protected_attr)):
            ...
            if k=='clfOrig' or k=='clfMit': #if clf==clfOrig or clf==clfMit:
            ...
            #selection_rates[protected_values[i]]=selection_rate(y_pred, y_test_r, pos_label=0)
        metric_score={
            'Precision':precisions,
            'Recall':recalls,
            #'Selection Rate':selection_rates
        }
        metric_scores.append(metric_score) #['Orig','OrigBlind','Mit','MitBlind']

    # https://matplotlib.org/3.1.1/gallery/lines_bars_and_markers/barchart.html#sphx-glr-gallery-lines-bars-and-markers-barchart-py
    for m in ['Precision','Recall']:#,'Selection Rate']:
        labels = protected_values

        x = np.arange(len(protected_values))  # the label locations
